      ...
      <div style={{ borderTop: '1px solid #E5E5E5' }}>
        {brand.typeScale.map(s => (
          <div key={s.name} style={{ display: 'grid', gridTemplateColumns: '80px 200px 1fr', gap: 16, padding: '16px 0', borderBottom: '1px solid #E5E5E5', alignItems: 'center' }}>
            <div style={{ fontFamily: 'Gellix, sans-serif', fontSize: 11, color: '#4D4D4D' }}>{s.size}</div>
            <div style={{ fontFamily: 'Gellix, sans-serif', fontSize: 11, color: '#4D4D4D' }}>{s.name} · {s.family} {s.weight}</div>
